...
#include <vector>
#include <algorithm>
#include <memory> //za shared_ptr i unique_ptr
...
void UnesiUcenike(std::vector<std::shared_ptr<Ucenik>> &p_ucenici) {
  ...
  for (int i = 0; i < p_ucenici.size(); i++) {
    ...
    p_ucenici[i] = std::make_shared<Ucenik>(); //umjesto = new int(); moglo je i (new Ucenik()) bez =
...
void ObradiUcenike(std::vector<std::shared_ptr<Ucenik>> &p_ucenici) {
  ...
  std::sort(p_ucenici.begin(), p_ucenici.end(),
            [](std::shared_ptr<const Ucenik> p_u1, std::shared_ptr<const Ucenik>p_u2) { //vazna napomena const Ucenik* => std::shared_ptr<const Ucenik>(objekat konstantan) , a NE const std::shared_ptr<Ucenik> (pametni pokazivac konstantan)
...
void IspisiIzvjestaj(const std::vector<std::shared_ptr<Ucenik>> &p_ucenici) { //ovdje je vec VEKTOR konstantan nevezano za pokazivac
...
/* Ova funkcija nam sad ne treba jer koristimo pametne pokazivace
