...
private:
    ...
    Student* kod_koga_je=nullptr; //OVDJE NAME NE TREBA shared_ptr (niti moze -> javlja gresku zbog operatora = da ne prima shared ptr)
public:
...
void Laptop::Zaduzi(Student& s) {
    if(kod_koga_je) throw std::domain_error("Laptop vec zaduzen");// !zaduzen je za nullptr
    kod_koga_je=&s;
}
void Laptop::Razduzi() {
    kod_koga_je=nullptr;
...
private:
    std::map<int,std::shared_ptr<Student>> studenti; //indeks kljucno polje
    std::map<int,std::shared_ptr<Laptop>> laptopi; //ev_broj kljucno polje
public:
...
void Administracija::RegistrirajNovogStudenta(int ind,std::string gs,std::string ip,std::string adr,std::string broj) {
        ...
        if(it==studenti.end()) { //provjera da li ne postoji jer izraz mapa[kljucno_polje] automatski stvara novi objekt, pa moram vrsit provjeru da ovo izbjegnem(a mijenja stari ako takav postoji)
           // Student* pq=new Student(ind,gs,ip,adr,broj);
            ...
            try {
                studenti[ind]=std::make_shared<Student>(ind,gs,ip,adr,broj);
            }
            catch(...) {
                throw;
            }
...
void Administracija::RegistrirajNoviLaptop(int eb,std::string n,std::string k) {
    ...
    if(!laptopi.count(eb)) {
        ...
        try {
            laptopi[eb]=std::make_shared<Laptop>(eb,n,k);
        }
        catch(...) {
            throw;
        }
...
void Administracija::IzlistajLaptope() const {
    ...
    if((it->second)->DaLiJeZaduzen()) {
        std::cout<<"Zaduzio(la): ";
        Student* kod_koga_je=(it->second)->DajPokKodKogaJe();
        std::cout<<kod_koga_je->DajImePrezime()<<" ("<<kod_koga_je->DajIndeks()<<")"<<std::endl;
    }
    std::cout<<std::endl;
    }
}
...
try {
    Administracija etf;
        ...
        for(;;) {
            ...
            std::cout<<"10 - Kraj programa"<<std::endl<<std::endl;
            std::cin>>opcija;
            if(opcija==1) {
                int ind;
