...
private:
    double koordinate[3];
    mutable int brojac=0; //ovim mutable naznacimo da iako je privatan moze se mijenjati u const funkcijama
public:
    ...
    //treba ubacit kopirajuci konstruktor(on se automatski stvara u basic formi ako ga nema) ali treba ga konstruisat ako hocemo neke izmjene ko odje
    Vektor3d(const Vektor3d &vektor) {
        koordinate[0]=vektor.koordinate[0];
        koordinate[1]=vektor.koordinate[1];
        koordinate[2]=vektor.koordinate[2];
        brojac=0;
    }
